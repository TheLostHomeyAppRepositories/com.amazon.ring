...
const { RingApi } = require('../ring-client-api') // 13.1.0 10-09-2024 Stops at this.locations = await this.ringApi.getLocations();
...
class Api extends Homey.SimpleClass {
    ...
    async _subLocations(locations) {
        ...
        locations.forEach(async location => {
            ...
            // Get Chimes on the location
            this.log('_subLocations:             Subscribing to chimes');
            await this._getChimes(location)

            // Get Intercoms on the location
            this.log('_subLocations:             Subscribing to intercoms');
            ...
            */
            ...
            if (location.hasAlarmBaseStation) {
                this.log('_subLocations:             Ring Alarm present at',location.locationDetails.name);
                ...
                this.log('_subLocations:             No Ring Alarm present at',location.locationDetails.name);
    ...
    async _subscribeToAlarmMode(location) {
        ...
        this.log('_subscribeToAlarmMode:     Alarm Mode subscribed for location:', location.name);
    }
    ...
    async _getAlarmDevices(location) {
        ...
        try {
                    ...
                    device.onData.subscribe((data) => {
                        ...
                        try {
                            if (this._authenticated) {
                                ...
                                this.log('_getAlarmDevices:          Alarm Devices subscribed for location:', location.name);
                            }
                        }
                        catch(error) {
                            this.log('_getAlarmDevices:          Error:', location.name, error);
                        }
            ...
            this.log('_getAlarmDevices:          Error _getIntercoms', error);
        }
    }
        ...
        location.onLocationMode.subscribe((locationMode) => {
            ...
            try {
                if (this._authenticated) {
                    ...
                    this.log('_subscribeToLocationMode:  Location Mode subscribed for location:', location.name);
                }
            } catch (error) {
    ...
    async _getChimes(location) {
        ...
        try {
            ...
            this.log('_getChimes:                Chimes subscribed for location:', location.name);
        }
        catch (error) {
    ...
    async _getIntercoms(location) {
        ...
        try {
            ...
            this.log('_getIntercoms:             Intercoms subscribed for location:', location.name);
        }
        catch (error) {
