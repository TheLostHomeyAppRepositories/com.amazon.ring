        ...
        let request = https.request(url, (response) => {
            ...
            response.on('end', () => {
                ...
                if (response.statusCode >= 400) {
                    ...
                    this.homey.app.writeLog(logLine);
                    try {
                        // todo handle data = "<html><head><title>406 Not Acceptable</title></head><body bgcolor="white"><center><h1>406 Not Acceptable</h1></center><hr><center>nginx</center></body></html>""
                        report = JSON.parse('{"error": "invalid_response ' + response.statusCode + '", "error_description": "' + data + '"}');
                    } catch (e) {                        
                        report = JSON.parse(data);
                    }
            ...
            if (error) {
                ...
                this._https_auth_mfa_refresh((error, result) => {
                    if (error) {
                        ...
                        this._bearer = result.access_token;
                        this._https('GET', client_api_host, '/clients_api/snapshots/image/' + device_data.id, null, true, (error, result) => {
                            logLine = " Api.js || " + "grabImage || " + "grabImage _https_auth_mfa_refresh authenticated succesfully";
                            this.homey.app.writeLog(logLine);
                            this.log("grabImage _https_auth_mfa_refresh authenticated succesfully. Result is probably no image",result );
                            return callback(error, result);
                        });
                    }
                });
                ...
                this.cameras.forEach(async (camera) => {            
                    if (camera.initialData.id == device_data.id) {
                        this.log('Device is',camera.initialData.description)
