...
class Api extends Homey.SimpleClass {
    ...
    async _connectRingAPI() {    
        ...
        this.ringApi = new RingApi({
            ...
            cameraStatusPollingSeconds: refreshDeviceInterval/1000,         //Interval is stored as milliseconds, API uses seconds
            locationModePollingSeconds: refreshLocationModeInterval/1000,   //Interval is stored as milliseconds, API uses seconds
        ...
        // subscribe to recieve new refresh token, to be implemented
        this.ringApi.onRefreshTokenUpdated.subscribe(
            async ({ newRefreshToken, oldRefreshToken }) => {
                // this.log("Refresh Token Updated: ", newRefreshToken);
                if (!oldRefreshToken) {
                    return;
                }
                this.log('New refreshtoken recieved and saved to settings')
                this.homey.settings.set('ringRefreshToken', newRefreshToken);
            }
        );
        ...
        this.locations.forEach(async location => {
            location.onLocationMode.subscribe((locationMode) => {
                ...
                this.emit('refresh_locationMode', locationInfo);
            })
        });
        ...
        this.cameras.forEach(async camera => {
            ...
            camera.onData.subscribe((data) => { // this function can replace _syncDevice(s)
                //this.log('data',data);
                this.emit('ringOnData', data);
            })
        });
        
        // await this.chimes.playSound('ding')
        // await this.chimes.playSound('motion')
    ...
    // todo: new functions for Chime
    // snoozeChime(device_data, time, callback){ chime.snooze(time) } // time is in minutes, max 24 * 60 (1440)
    // unSnoozeChime(device_data, callback){ chime.clearSnooze() }
    ...
    /* replace setLocationMode(), mapLocationMode() and _setLocationMode
    setLocationMode(locationId, mode) {
        return new Promise(async (resolve, reject) => {
          try {
            const location = this.locations.find(l => l.locationDetails.location_id === locationId);
            if (!location) {
              throw new Error(`Location ${locationId} not found`);
            }
            const result = await location.setLocationMode(mode);
            resolve(result);
          } catch (error) {
            this.error(`setLocationMode: error with result:`, error.message);
            reject(error);
          }
        });
    }
