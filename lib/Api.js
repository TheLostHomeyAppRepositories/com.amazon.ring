...
class Api extends Homey.SimpleClass {
    ...
    _setAuthenticationStatus (status, report) {
        ...
        if (this._previousAuthenticated != this._authenticated) {
            if (this._authenticated) {
                // at boot: undefined 
                this.log('_setAuthenticationStatus: this.NoAuthStatLogTimeout', this.NoAuthStatLogTimeout);
                    if (this.NoAuthStatLogTimeout) {this.log('Timeout true')} else {this.log('Timeout false')}
                clearTimeout(this.NoAuthStatLogTimeout);
                this.log('_setAuthenticationStatus: this.NoAuthStatLogTimeout after clear', this.NoAuthStatLogTimeout);
                    if (this.NoAuthStatLogTimeout) {this.log('Timeout true')} else {this.log('Timeout false')}
