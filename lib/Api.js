...
class Api extends Homey.SimpleClass {
   ...
   async _refreshAlarmMode (locationId) {
    ...
    async _https_auth_cred(auth) {
        ...
        try {
            ...
            if (response.status >= 400) {
                ...
                throw new Error(`invalid_authentication ${response.status} ${data}`);
            }
    ...
    // This code can not be modernized, Homey requires a callback fro the UI to go to the next step after a succesful authentication with MFA
    _https_auth_code(auth, code, callback) {
        this.log('_https_auth_code');

        if (!auth) return callback(new Error('invalid_credentials'));
        ...
        fetch(url, options)
            .then(async response => {
                ...
                if (response.status >= 400) {
                    ...
                    error = new Error(`invalid_authentication ${response.status} ${data}`);
                    try {
                        report = JSON.parse(`{"error": "invalid_response ${response.status}", "error_description": "${data}"}`);
                    } catch {
                        report = { error: `invalid_response ${response.status}`, error_description: String(response.status) };
