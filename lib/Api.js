...
class Api extends Homey.SimpleClass {
    ...
    async _connectRingAPI() {
        ...
        try {
            ...
            // next line seems to never return on ring-client-api from 13.0.0 and up
            ...
            this.locations.forEach(async location => {
                ...
                if (location.hasAlarmBaseStation) {
                    ...
                    const keypad = devices.find(
                        (device) => device.data.deviceType === RingDeviceType.Keypad
                    )

                    this.log('baseStation               ',baseStation);
                    this.log('2 ------------------------------')
                    this.log('Keypad                    ',keypad);
                    this.log('3 ------------------------------')
