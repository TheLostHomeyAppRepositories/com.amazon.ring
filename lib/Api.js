...
class Api extends Homey.SimpleClass {
    ...
    async _connectRingAPI() {    
        ...
        this.ringApi = new RingApi({
            ...
            cameraStatusPollingSeconds: refreshDeviceInterval/1000,
            //locationModePollingSeconds: refreshLocationModeInterval/1000,
        ...
        // await this.chimes[0].playSound('motion')
    ...
    ringChime (device_data, callback) {
        ...
        this.chimes.forEach(async (chime)=>{
            if (chime.id == device_data.id) {
                this.log('matched',device_data.id);
                await this.chimes[0].playSound('ding')
                callback(null,true);
            }
        });
        */
