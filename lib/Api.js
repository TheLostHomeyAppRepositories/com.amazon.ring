...
const { RingApi } =require('../ring-client-api') // 11.7.5 07-04-2023
...
class Api extends Homey.SimpleClass {
        ...
        try {
            ...
            this.locations.forEach(async location => {
                ...
                if (location.hasAlarmBaseStation) {
                    ...
                    //const subscr = location.onLocationMode.subscribe((locationMode) => {
        ...
        try {
            ...
            this.cameras.forEach(async camera => {
                ...
                try {
                    ...
                    this.log('camera.onData', error);
                }
                // extra logging te determine camera settings
                //this.log ('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
                //this.log (camera);
                //this.log ('camera.initialData.subscribed',camera.initialData.subscribed);
                //this.log ('-----------------------------------------------------------------');
    ...
    _setAuthenticationStatus (status, report) {
        ...
        if (this._previousAuthenticated != this._authenticated) {
            if (this._authenticated) {
                ...
                //this.ringApi.restClient.authOptions.cameraStatusPollingSeconds = cameraStatusInterval;
                //this.ringApi.restClient.authOptions.locationModePollingSeconds = locationModeInterval;
                ...
                this.NoAuthStatLogTimeout = setTimeout(async () => {
                    ...
                    //this.ringApi.restClient.authOptions.cameraStatusPollingSeconds = 0;
                    //this.ringApi.restClient.authOptions.locationModePollingSeconds = 0;
    ...
    userLocations() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const locationObject = {user_locations: locationDetailsArray};

            this.log('userLocations locationObject',locationObject);
            
            const locations = locationObject.user_locations.map(location => {
                return {
                    name: location.name,
