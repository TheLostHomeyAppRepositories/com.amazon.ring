...
const { loadRingApi } = require('./utils');

//const { RingApi } = require('../ring-client-api') // 11.7.7 28-05-2023 // 11.7.6 26-05-2023 // 11.7.5 07-04-2023 
//const { RingApi } = require('../ring-client-api12.1.1') // 12.1.1 11-09-2024
//const { RingApi } = require('../ring-client-api13.1.0') // 13.1.0 10-09-2024 Stops at this.locations = await this.ringApi.getLocations();

const { RingDeviceType } = require('../ring-client-api13.1.0');
...
const alarmDeviceList = [ 'security-panel', 'sensor.contact', 'sensor.motion', 'security-keypad', 'hub.redsky' ];
...
class Api extends Homey.SimpleClass {
    ...
    async _connectRingAPI() {
        ...
        const { RingApi } = loadRingApi(this.homey.platformVersion);
        ...
        this.log('_connectRingAPI:           Refresh Token updates subscribed');
        ...
        try {
            ...
            // next line seems to never return on ring-client-api from 13.0.0 and up, this is caused by an old NodeJS version on Homey
            this.locations = await this.ringApi.getLocations();

            this.log('_connectRingAPI:           locations retrieved');
            ...
            this.locations.forEach(async location => {
                ...
                if (location.hasAlarmBaseStation) {
                    ...
                    this.log('_connectRingAPI:           Ring Alarm present at',location.locationDetails.name);
                    this.homey.setInterval(() => this._refreshAlarmMode(location.locationDetails.location_id), (alarmModeInterval*1000));
   
                    this.log('Location alarm mode       ',await location.getAlarmMode());
                    ...
                    this.devices.forEach(async device => {
                        if (alarmDeviceList.indexOf(device.data.deviceType) >= 0) {
                            // this.log(device.data.name+":",device.data.deviceType);
                            ...
                            if ( device.data.deviceType == RingDeviceType.SecurityPanel ) {
                                ...
                                this.homey.app.alarmSystem.location.id = location.locationDetails.location_id;
        ...
        try {
            ...
            this.cameras.forEach(async camera => {
                ...
                camera.onData.subscribe((data) => {
                    // this.log('camera.onData',data.description);
        ...
        try {
            ...
            this.intercoms.forEach(async (intercom) => {
                intercom.onDing.subscribe((intercom) => {
                    this.log('Api.js onDing', intercom);
                    try {
                        if (this._authenticated) {
                ...
                intercom.onData.subscribe((data) => {
                    this.log('Api.js camera.onData',data);
                    if (this.ringApi && !this.ringApi.restClient.refreshToken) {
                        this.log('onData:                   refreshToken lost');
